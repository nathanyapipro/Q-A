...
grant select on app_public.user to fundamental_unauthenticated, fundamental_anonymous, fundamental_master;
-- NOTE: `insert` is not granted, because we'll handle that separately
grant update(username) on app_public.user to fundamental_anonymous, fundamental_master;
grant delete on app_public.user to fundamental_anonymous, fundamental_master;
